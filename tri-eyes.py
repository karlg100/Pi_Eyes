...
domC               = parse("graphics/cyclops-eye.svg")
vbC                = get_view_box(domC)
pupilMinPtsC       = get_points(domC, "pupilMin"      , 32, True , True )
pupilMaxPtsC      = get_points(domC, "pupilMax"      , 32, True , True )
irisPtsC           = get_points(domC, "iris"          , 32, True , True )
scleraFrontPtsC    = get_points(domC, "scleraFront"   ,  0, False, False)
scleraBackPtsC     = get_points(domC, "scleraBack"    ,  0, False, False)
upperLidClosedPtsC = get_points(domC, "upperLidClosed", 33, False, True )
upperLidOpenPtsC   = get_points(domC, "upperLidOpen"  , 33, False, True )
upperLidEdgePtsC   = get_points(domC, "upperLidEdge"  , 33, False, False)
lowerLidClosedPtsC = get_points(domC, "lowerLidClosed", 33, False, False)
lowerLidOpenPtsC   = get_points(domC, "lowerLidOpen"  , 33, False, False)
lowerLidEdgePtsC   = get_points(domC, "lowerLidEdge"  , 33, False, False)
...
eyePosition = (DISPLAY.width / 4) + 90
eyePositionC = 0
#eyePositionC = (DISPLAY.width / 2)
...
scale_points(pupilMinPtsC      , vbC, eyeRadius)
scale_points(pupilMaxPtsC      , vbC, eyeRadius)
scale_points(irisPtsC          , vbC, eyeRadius)
scale_points(scleraFrontPtsC   , vbC, eyeRadius)
scale_points(scleraBackPtsC    , vbC, eyeRadius)
scale_points(upperLidClosedPtsC, vbC, eyeRadius)
scale_points(upperLidOpenPtsC  , vbC, eyeRadius)
scale_points(upperLidEdgePtsC  , vbC, eyeRadius)
scale_points(lowerLidClosedPtsC, vbC, eyeRadius)
scale_points(lowerLidOpenPtsC  , vbC, eyeRadius)
scale_points(lowerLidEdgePtsC  , vbC, eyeRadius)
...
centerIris = mesh_init((32, 4), (0, 0.5 / irisMap.iy), True, False)
centerIris.set_textures([irisMap])
centerIris.set_shader(shader)
...
centerUpperEyelid = mesh_init((33, 5), (0, 0.5 / lidMap.iy), False, True)
centerUpperEyelid.set_textures([lidMap])
centerUpperEyelid.set_shader(shader)
centerLowerEyelid = mesh_init((33, 5), (0, 0.5 / lidMap.iy), False, True)
centerLowerEyelid.set_textures([lidMap])
centerLowerEyelid.set_shader(shader)
...
centerEye = pi3d.Lathe(path=pts, sides=64)
centerEye.set_textures([scleraMap])
centerEye.set_shader(shader)
re_axis(centerEye, 0)
...
startXC      = random.uniform(-30.0, 30.0)
n            = math.sqrt(900.0 - startX * startX)
startYC      = random.uniform(-n, n)
destXC       = startXC
destYC       = startYC
curXC        = startXC
curYC        = startYC
moveDurationC = random.uniform(0.075, 0.175)
holdDurationC = random.uniform(0.1, 1.1)
startTimeC    = 0.0
isMovingC     = False
...
centerEye.positionX(eyePositionC)
centerIris.positionX(eyePositionC)
centerUpperEyelid.positionX(eyePositionC)
centerUpperEyelid.positionZ(-eyeRadius - 42)
centerLowerEyelid.positionX(eyePositionC)
centerLowerEyelid.positionZ(-eyeRadius - 42)
...
prevCenterUpperLidWeight  = 0.5
prevCenterLowerLidWeight  = 0.5
...
prevCenterUpperLidPts  = points_interp(upperLidOpenPts, upperLidClosedPts, 0.5)
prevCenterLowerLidPts  = points_interp(lowerLidOpenPts, lowerLidClosedPts, 0.5)
...
blinkDurationCenter  = 0.1
blinkDurationRight  = 0.1
blinkStartTimeLeft  = 0
blinkStartTimeCenter = 0
...
def frame(p):
	...
	global startXC, startYC, destXC, destYC, curXC, curYC
	global startXR, startYR, destXR, destYR, curXR, curYR
	global moveDuration, holdDuration, startTime, isMoving
	global moveDurationC, holdDurationC, startTimeC, isMovingC
	global moveDurationR, holdDurationR, startTimeR, isMovingR
	global frames
	global leftIris, centerIris, rightIris
	global pupilMinPts, pupilMaxPts, irisPts, irisZ
	global leftEye, centerEye, rightEye
	global leftUpperEyelid, leftLowerEyelid, centerUpperEyelid, centerLowerEyelid, rightUpperEyelid, rightLowerEyelid
	global upperLidOpenPts, upperLidClosedPts, lowerLidOpenPts, lowerLidClosedPts
	global upperLidEdgePts, lowerLidEdgePts
	global prevLeftUpperLidPts, prevLeftLowerLidPts, prevCenterUpperLidPts, prevCenterLowerLidPts, prevRightUpperLidPts, prevRightLowerLidPts
	global leftUpperEyelid, leftLowerEyelid, centerUpperEyelid, centerLowerEyelid, rightUpperEyelid, rightLowerEyelid
	global prevLeftUpperLidWeight, prevLeftLowerLidWeight, prevCenterUpperLidWeight, prevCenterLowerLidWeight, prevRightUpperLidWeight, prevRightLowerLidWeight
	global prevPupilScale
	global irisRegenThreshold, upperLidRegenThreshold, lowerLidRegenThreshold
	global luRegen, llRegen, cuRegen, clRegen, ruRegen, rlRegen
	global timeOfLastBlink, timeToNextBlink
	global blinkStateLeft, blinkStateCenter, blinkStateRight
	global blinkDurationLeft, blinkDurationCenter, blinkDurationRight
	global blinkStartTimeLeft, blinkStartTimeCenter, blinkStartTimeRight
	global trackingPos
	global trackingPosC
	global trackingPosR
...
#		print(frames/(now-beginningTime))
	...
	if CRAZY_EYES:
            ...
            if isMovingC == True:
                if dtC <= moveDurationC:
                    scale        = (now - startTimeC) / moveDurationC
                    # Ease in/out curve: 3*t^2-2*t^3
                    scale = 3.0 * scale * scale - 2.0 * scale * scale * scale
                    curXC        = startXC + (destXC - startXC) * scale
                    curYC        = startYC + (destYC - startYC) * scale
                else:
                    startXC      = destXC
                    startYC      = destYC
                    curXC        = destXC
                ...
                if dtC >= holdDurationC:
                    destXC        = random.uniform(-30.0, 30.0)
                    n             = math.sqrt(900.0 - destXC * destXC)
                    destYC        = random.uniform(-n, n)
                    moveDurationC = random.uniform(0.075, 0.175)
                    startTimeC    = now
                    isMovingC     = True
	...
	if AUTOBLINK and (now - timeOfLastBlink) >= timeToNextBlink:
		...
		if blinkStateCenter != 1:
			blinkStateCenter     = 1 # ENBLINK
			blinkStartTimeCenter = now
			blinkDurationCenter  = duration
	...
	if blinkStateCenter: # Left eye currently winking/blinking?
		# Check if blink time has elapsed...
		if (now - blinkStartTimeCenter) >= blinkDurationCenter:
			# Yes...increment blink state, unless...
			if (blinkStateCenter == 1 and # Enblinking and...
			    ((BLINK_PIN >= 0 and    # blink pin held, or...
			      GPIO.input(BLINK_PIN) == GPIO.LOW) or
			    (WINK_L_PIN >= 0 and    # wink pin held
			      GPIO.input(WINK_L_PIN) == GPIO.LOW))):
				...
				blinkStateCenter += 1
				if blinkStateCenter > 2:
					blinkStateCenter = 0 # NOBLINK
				else:
					blinkDurationCenter *= 2.0
					blinkStartTimeCenter = now
	else:
		if WINK_L_PIN >= 0 and GPIO.input(WINK_L_PIN) == GPIO.LOW:
			blinkStateCenter     = 1 # ENBLINK
			blinkStartTimeCenter = now
			blinkDurationCenter  = random.uniform(0.035, 0.06)
	...
	if TRACKING:
		...
		if CRAZY_EYES:
			...
			trackingPosC = (trackingPosC * 3.0 + n) * 0.25
			n = 0.4 - curYR / 60.0
			if   n < 0.0: n = 0.0
			elif n > 1.0: n = 1.0
	...
	if blinkStateCenter:
		n = (now - blinkStartTimeCenter) / blinkDurationCenter
		if n > 1.0: n = 1.0
		if blinkStateCenter == 2: n = 1.0 - n
	else:
		n = 0.0
	newCenterUpperLidWeight = trackingPos + (n * (1.0 - trackingPos))
	newCenterLowerLidWeight = (1.0 - trackingPos) + (n * trackingPos)
	...
	if blinkStateRight:
		...
		n = 0.0
	if CRAZY_EYES:
		newCenterUpperLidWeight = trackingPosC + (n * (1.0 - trackingPosC))
		newCenterLowerLidWeight = (1.0 - trackingPosC) + (n * trackingPosC)
	...
	if (cuRegen or (abs(newCenterUpperLidWeight - prevCenterUpperLidWeight) >=
	  upperLidRegenThreshold)):
		newCenterUpperLidPts = points_interp(upperLidOpenPts,
		  upperLidClosedPts, newCenterUpperLidWeight)
		if newCenterUpperLidWeight > prevCenterUpperLidWeight:
			centerUpperEyelid.re_init(pts=points_mesh(
			  (upperLidEdgePts, prevCenterUpperLidPts,
			  newCenterUpperLidPts), 5, 0, False))
		else:
			centerUpperEyelid.re_init(pts=points_mesh(
			  (upperLidEdgePts, newCenterUpperLidPts,
			  prevCenterUpperLidPts), 5, 0, False))
		prevCenterUpperLidPts    = newCenterUpperLidPts
		prevCenterUpperLidWeight = newCenterUpperLidWeight
		cuRegen = True
	else:
		cuRegen = False

	if (clRegen or (abs(newCenterLowerLidWeight - prevCenterLowerLidWeight) >=
	  lowerLidRegenThreshold)):
		newCenterLowerLidPts = points_interp(lowerLidOpenPts,
		  lowerLidClosedPts, newCenterLowerLidWeight)
		if newCenterLowerLidWeight > prevCenterLowerLidWeight:
			centerLowerEyelid.re_init(pts=points_mesh(
			  (lowerLidEdgePts, prevCenterLowerLidPts,
			  newCenterLowerLidPts), 5, 0, False))
		else:
			centerLowerEyelid.re_init(pts=points_mesh(
			  (lowerLidEdgePts, newCenterLowerLidPts,
			  prevCenterLowerLidPts), 5, 0, False))
		prevCenterLowerLidWeight = newCenterLowerLidWeight
		prevCenterLowerLidPts    = newCenterLowerLidPts
		clRegen = True
	else:
		clRegen = False
	...
	# Center eye (center screen)
	if CRAZY_EYES:
		centerIris.rotateToX(curYC)
		centerIris.rotateToY(curXC - convergence)
		centerIris.draw()
		centerEye.rotateToX(curYC)
		centerEye.rotateToY(curXC - convergence)
	else:
		centerIris.rotateToX(curY)
		centerIris.rotateToY(curX - convergence)
		centerIris.draw()
		centerEye.rotateToX(curY)
		centerEye.rotateToY(curX - convergence)
	centerEye.draw()
	...
	centerUpperEyelid.draw()
	centerLowerEyelid.draw()
